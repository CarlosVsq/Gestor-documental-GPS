...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      /*
      TAREA POR HACER AL FINALIZAR BIEN 
      Descomentar para aplicar redirección automática según rol (HU-25) cuando todos los módulos estén completamente integrados.
      
      if (loggedUser.rol === 'colaborador' || loggedUser.rol === 'auditor') {
        targetPage = 'documentos';
      } else if (loggedUser.rol === 'supervisor') {
        targetPage = 'proyectos';
      }
      */

      navigate('/', { replace: true, state: { initialPage: targetPage } });
    } catch (err: any) {
